...
<nav class="navbar navbar-expand-lg navbar-light shadow">
    ...
    if (loginDetails != null && !loginDetails.isEmpty()) {
        admin = loginDetails.get(0); // Get the first user (assuming unique username)
    %>
    ...
    <p><strong>Birthday:</strong> <%= admin.getBirthday() %></p>
    <p><strong>Contact No:</strong> <%= admin.getPhone() %></p>
    <p><strong>Username:</strong> <%= admin.getUsername() %></p>
    <p><strong>Password:</strong> <%= admin.getPassword() %></p>
    <%
    } else {
    %>
    <p>No details found. Please check your profile information.</p>
    <a href="Home.jsp">Go to Profile</a>
    <%
    }
    %>
</div>
<% if (admin != null) { %>
<!-- Generate update URL with user details -->
<c:url var="updateuser" value="userprofileupdate.jsp">
    <c:param name="iduser" value="<%= String.valueOf(admin.getIduser()) %>"/>
    <c:param name="firstname" value="<%= admin.getFirstname() %>"/>
    <c:param name="lastname" value="<%= admin.getLastname() %>"/>
    <c:param name="email" value="<%= admin.getEmail() %>"/>
    <c:param name="birthday" value="<%= admin.getBirthday() %>"/>
    <c:param name="phone" value="<%= admin.getPhone() %>"/>
    <c:param name="username" value="<%= admin.getUsername() %>"/>
    <c:param name="password" value="<%= admin.getPassword() %>"/>
</c:url>

<!-- Update button with the generated URL -->
<a href="${updateuser}" class="btn btn-info btn-block">Edit Profile</a>
<% } %>
            ...
            <div class="col-md-4">
                <div class="side-section mt-5">
                    <img src="images/login.png" alt="Profile Picture" class="profile-picture img-fluid">
                    <h3 class="mt-3">Actions</h3>
                    <a href="bookingsearch.jsp" class="btn btn-success btn-block">Booking Details</a>
                    <a href="feedbacksearch.jsp" class="btn btn-success btn-block">Feedback Details</a>
                   
                   
                    <form action="guideallview" method="post">
				        <input type="submit" name="submit" value="Guide Details" class="btn btn-success btn-block" style="margin-top: 5px;">
				    </form>
                    <c:url value="Userdeleteservlet" var="userdelete">
    			<c:param name="iduser" value="${userID}"/>
				</c:url>

				<form action="${userdelete}" method="post">
				    <input type="hidden" name="iduser" value="${userID}"> <!-- Use hidden input to store the ID -->
				    <br>
				    <button type="submit" class="btn btn-danger btn-block">Delete Account</button>
				</form>
				<br>
				<a href="login.jsp" class="btn btn-secondary btn-block">Logout</a>
                </div>
            </div>
        </div>
    </div>
